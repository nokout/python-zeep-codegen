...
It orchestrates the complete pipeline:
  1. Generate Python dataclasses from XSD using xsdata
...
Usage:
    ...
    python wsdl_to_schema.py input.xsd --main-model Order --module custom.module
    python wsdl_to_schema.py input.xsd --main-model Order --models-dir generated_models
"""
import argparse
import subprocess
import sys
...
def generate_dataclasses(xsd_file: str, output_package: str = "models") -> str:
    """
    Generate Python dataclasses from XSD file using xsdata.
    ...
    Returns:
        Module name of generated dataclasses
    """
    print(f"\n{'='*70}")
    ...
    # Run xsdata generate command
    cmd = ["xsdata", "generate", "-p", output_package, xsd_file]
    ...
    print(f"  ✓ Generated dataclasses in {output_package}/")
    print(f"  ✓ Module: {full_module}")
    
    return full_module


def convert_to_pydantic(module_name: str) -> tuple[dict, Path]:
    """
    Convert dataclasses from module to Pydantic models.
    
    Args:
        module_name: Fully qualified module name (e.g., 'models.sample_complex')
...
Examples:
  ...
  python wsdl_to_schema.py input.xsd --main-model Order --module custom.models
...
"""
    ...
    parser.add_argument(
        '--module',
        help='Python module name for generated models (default: auto-detect from XSD filename)'
    )
    
    parser.add_argument(
        '--models-dir',
        default='models',
        help='Directory for generated dataclass models (default: models)'
    )
    ...
    parser.add_argument(
        ...
        help='Output path for Pydantic models (default: generated/pydantic_models.py)'
    )
    ...
    # Step 1: Generate dataclasses from XSD
    if args.module:
        # Use provided module name
        module_name = args.module
        print(f"\nUsing provided module: {module_name}")
        print(f"Note: Make sure to run xsdata manually if needed:")
        print(f"  xsdata generate -p {args.models_dir} {args.xsd_file}")
    else:
        # Auto-generate using xsdata
        module_name = generate_dataclasses(args.xsd_file, args.models_dir)
    ...
    print(f"{'='*70}")
    print(f"\nGenerated files:")
    print(f"  • Dataclasses: {args.models_dir}/")
